...
# Our dataframe needs to be extensively pre-processed before being put into a model.
# Our first goal will be to reduce the amount of features we are putting into 
# our model based on multicolinearity. 
#
# Afterwards, the data will need to be split into a test/train dataset. An imputation
# model will need to be developed for the test dataset and eventually applied to the
# training dataset before a final model is created.
#
# Written by Noor Tasnim on October 20, 2025
...
exergame <- readRDS("results/exergameWideForModel.RDS")
...
exergame_final <- exergame %>%
  ...
  select(-(anger_dsm:lower_injury)) %>% # improved performance and not studied in depth
  select(-weight,-height) %>% # we know there are sex differences, and males have greater weight/height in population
  ...
  select(-contains("low_beta")) %>% # account for harmonic/fake beta
  select(-contains("high_beta")) %>%
  select(-contains("gamma")) %>% # gamma cannot be reliably measured through scalp EEG
  # select(-(starts_with(c("DAN", "DMN")) & ends_with("gamma"))) %>%
  # select(-(starts_with(c("DAN", "DMN")) & ends_with("theta"))) %>%
  # select(-(starts_with(c("DAN", "DMN")) & ends_with("beta"))) %>%
...
# RDS File
saveRDS(exergame_final, file = "results/exergame_forFinalModel.rds")  
# CSV file
write.csv(exergame_final, file = "results/exergame_forFinalModel.csv", row.names = FALSE)


# OLD SCRIPT CHECKING MULTICOLINEARITY! -----------------------------------
