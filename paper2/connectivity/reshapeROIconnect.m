...
%
% --- EDITS ---
% Last Edit: November 16, 2025: to add session 2 connectivity matrices to
% analysis
% November 2, 2025: to account for exgm169_s1_gonogo IC weights

%% Load ROIconnect matrices
load ../data/roiconnect/session1/session1_vectors/connectivity_matrices_updated.mat
load ../data/roiconnect/session2/session2_vectors/s2_connectivity_matrices.mat

%% Remove data from participant dropouts in connectivity_matrices (Session 1)

participantsToRemove = {'exgm077', 'exgm152', 'exgm160', 'exgm175'};
% Find rows that do NOT contain these values
rowsToKeep = ~ismember(connectivity_matrices(:, 1), participantsToRemove);
% Keep only those rows
connectivity_matrices = connectivity_matrices(rowsToKeep, :);

s1_unique = unique(connectivity_matrices(:,1));
s2_unique = unique(s2_connectivity_matrices(:,1));

% Validation: check if Session 1 and Session 2 contains the same set of
% participants
if ~isempty(setdiff(s1_unique, s2_unique)) || ~isempty(setdiff(s2_unique, s1_unique))
    error('s1_unique and s2_unique do not contain the same values');
end

%% Stack connectivity matrix data from session 1 and session 2 

connectivity_matrices = [connectivity_matrices; s2_connectivity_matrices];
...
output_dir = '/Users/noor/Library/CloudStorage/GoogleDrive-ntasnim@vt.edu/Shared drives/Embodied Brain Lab/Exergame and ADHD (IRB 23-811)/! ROI Connect/session2/session2_figures/figures/roiconnect/DMN_DAN_matrices/';
...
% Write to CSV
output_filename = '/Users/noor/Library/CloudStorage/GoogleDrive-ntasnim@vt.edu/Shared drives/Embodied Brain Lab/Exergame and ADHD (IRB 23-811)/! Paper 2 Statistics/connectivity_analysis_results.csv';
writetable(output_table, output_filename);
fprintf('\nResults exported to %s\n', output_filename);

%% Save New Cell Array of Connectivity Data
save '/Users/noor/Library/CloudStorage/GoogleDrive-ntasnim@vt.edu/Shared drives/Embodied Brain Lab/Exergame and ADHD (IRB 23-811)/! Paper 2 Statistics/DMN_DAN_connectivity.mat' new_connectivity_data
