...
demographics_mh <- demographics_mh %>%
  mutate(elapsed_time = as.numeric(difftime(intervention_visit_arm_1,
...
# Export csv file of timespan between sessions
timespan <- demographics_mh %>%
  select(participant_id,elapsed_time)
write_csv(timespan,'results/time_between_sessions.csv')
