...
if (n_cores_available > 8) {
  # On HPC with many cores - use all
  n_cores <- n_cores_available
} else {
  # On local machine - leave one free
  n_cores <- n_cores_available - 1
}

cat(sprintf("\nDetected %d cores, using %d for parallel processing\n", 
...
evaluate_params <- function(params_row, exergame_data, n_splits = 20, n_folds = 5) {
